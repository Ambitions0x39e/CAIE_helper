...
def test_parse_grading_result_valid():
    raw = json.dumps({
        "question": "Q1",
...
def test_parse_grading_result_strips_code_fences():
    raw = '```json\n{"question":"Q2","marks":[],"total":0,"max":3,"comment":"empty"}\n```'
    result = parse_grading_result(raw)
    assert result.question == "Q2"


def test_parse_grading_result_invalid_json():
    with pytest.raises(ValueError, match="Failed to parse"):
        parse_grading_result("not json at all")


def test_parse_grading_result_missing_fields():
    with pytest.raises(ValueError, match="Missing fields"):
        parse_grading_result('{"question": "Q1"}')


def test_question_result_model():
    qr = QuestionResult(
        question="Q1",
