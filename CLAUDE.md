...
界面上每多一句解释，都是在替读者假设他看不懂。「跟随系统跟着 macOS /
Windows 的浅色深色走」「选定后一直有效，关掉窗口也记得」—— 这两句都是
一个写着「跟随系统 / 浅色 / 深色」的三选一控件本来就说清楚了的事。

不允许自动补上的东西：控件下方的补充说明、页面末尾那句「贴心提示」、
把标签又用一句话重讲一遍的 hint、承诺行为的免责话术。

控件的标签和选项文字就是它的说明。说不清楚就把标签改好，不要在旁边
再挂一句。用户明确要求写提示时才写。
